	...
	extensions: [
		{
			...
			renderer(rawToken) {
				...
				if (type === "note" || type === "note:") {
					return `<blockquote class="alert-note">${marked.parse(token.text)}</blockquote>`;
				}
				if (type === "tip" || type === "tip:") {
					return `<blockquote class="alert-tip">${marked.parse(token.text)}</blockquote>`;
				}
				if (type === "warning" || type === "warning:") {
					return `<blockquote class="alert-warning">${marked.parse(token.text)}</blockquote>`;
				}
				return false;
			},
		},
			...
			renderer(rawToken) {
				...
				if (options.linenumber) {
					if (options.linenumber === true) {
						...
						const n = Number.parseInt(options.linenumber);
				...
				return `<div class="codeblock ${type ? `codeblock-${type}` : ""}">${header ? `<div class="codeblock-header">${header}</div>` : ""}<div class="codeblock-body">${
					linenumber
...
for (const post of allPosts) {
	...
	for (const entry of fs.readdirSync(join(__dirname, "..", "blog", post.slug), {
		withFileTypes: true,
	})) {
